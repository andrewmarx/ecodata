...
This package is a little unusual in that it can create datasets from user-contributed functions. These functions can create datasets based on zero, one, or more data files. Because of this behavior, this page treats a *"data file"* and a *"dataset"* as two different things. A *data file* is, as the name implies, a file containing data. It gets imported into the package and documented like a normal data package. A *dataset* is more flexible, and is defined using a function. This function allows it to dynamically produce results, like subsetting a data file so that it only keeps a small set of variables needed for a particular topic.

Datasets are created by updating the `./R/dataset.R` file, and then categorized using the files in the `./data-raw/` directory that start with an underscore. These files are what make it possilbe to take a single dataset and assign it to various subdisciplines and topics. For those familiar with databases, these files essentially act as a mini-database managing the many-to-many relationships between *subdisciplines*, *topics*, and *datasets*. 
...
Finally, to help keep file sizes small, consider rounding when there are many decimal values with a large number of trailing digits. This often occurs when the values are calculated and the entire result is kept regardless of significant digits. And changes from an original source should be noted in the file documentation.
...
The package automatically generates a base dataset for each data file. This dataset will have the same name as the file.

If you wish to create additional datasets, they must be defined as an entry in `./R/datasets.R`. The name must be in title case, and it muse have a description, a function definition, and a list of data file names used (if any).


# Categorization

Categorization is handled in by the csv files starting with an underscore in the `./data-raw/` directory. Every dataset (including the one auto-generated internally for the data file) must be added to one or more topics in the `_td_pivot.csv` file. If a topic doesn't exist, it can be added the `_topics.csv` file, and must have one or more entries added to the `_td_pivot.csv` and `_st_pivot.csv` files. If a subdiscipline entry does not exist, it can be added to the `_subdisciplines.csv` and `_st_pivot.csv` files.
